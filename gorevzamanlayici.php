...
<?php 
    ...
    $hafta = $row['haftanin_gunu']; // Normalde -1 gösterecek onun yerine başka birşey belirleyebilirsiniz
    }
    ...
    $secili = isset($_GET['edit']) && $_GET['edit'] == $row['id'] ? ' style="background-color: rgb(255, 235, 144); border-bottom: thin solid;"' : '';
    echo '
        <tr'.$secili.'>
            <td>'.near_date($row['sonraki_calisma']).'</td>
            <td>'.$hafta.'</td>
            <td>'.$row['gun'].'</td>
            <td>'.$row['saat'].'</td>
            <td>'.$row['dakika'].'</td>
            <td>'.$row['dosya_adi'].'</td>
            <td>'.$row['aktif'].'</td>
            <td>'.$row['gunluk_kayit'].'</td>
            <td>'.$row['gorev_adi'].'</td>
            <td style="text-align: center;"><a href="?edit='.$row['id'].'#edit"><span title="Görevi düzenlemek için tıkla" class="glyphicon glyphicon-edit"></span></a></td>
            <td style="text-align: center;"><a href="#" style="cursor: pointer;"><span data-name="'.strip_tags($row['gorev_adi']).'" id="veri_sil_'.$row['id'].'" title="Görevi silmek için tıkla" class="glyphicon glyphicon-remove"></span></a></td>
  ...
  <?php    
    if(isset($_GET['edit'])){
        ...
        if(!empty($veritabani_aktif)){
            echo '<script>jQuery(function($) { $("#veritabani_tablo").show(); });</script>';
        }
...
?>
    ...
    <tr>
        <td colspan="2">Zip Arşivin dosya adını değiştirebilirsiniz</td>
        <td colspan="4" style="padding: 0rem 0.75rem 0rem 0.75rem;vertical-align: middle;"><input type="text" class="form-control" value="<?php echo $editrow['secilen_yedekleme_oneki']; ?>" name="dizin_secilen_yedekleme_oneki" style="width:350px;" /></td>
        <td colspan="2">Yedeklenecek veri tabanına boşluksuz sadece Latin karakter ile bir isim vermelisiniz</td>
    </tr>
...
?>
    ...
    <tr>
        <td colspan="2">Veri tabanı Yedeğin Öneki</td>
        <td colspan="4" style="padding: 0rem 0.75rem 0rem 0.75rem;vertical-align: middle;"><input type="text" class="form-control" name="veritabani_secilen_yedekleme_oneki" id="veritabani_secilen_yedekleme_oneki" value="<?php echo $secilen_yedekleme_oneki; ?>" style="width:350px;" /></td>
        <td colspan="2">Yedeklenecek veri tabanına boşluksuz sadece Latin karakter ile bir isim vermelisiniz</td>
    </tr>
...
?>
    ...
    <tr>
        <td colspan="2">Zip Arşivin dosya adını değiştirebilirsiniz</td>
        <td colspan="4" style="padding: 0rem 0.75rem 0rem 0.75rem;vertical-align: middle;"><input type="text" class="form-control" name="dizin_secilen_yedekleme_oneki" style="width:350px;" /></td>
        <td colspan="2">Yedeklenecek veri tabanına boşluksuz sadece Latin karakter ile bir isim vermelisiniz</td>
    </tr>
...
?>
    ...
    <tr>
        <td colspan="2">Veri tabanı Yedeğin Öneki</td>
        <td colspan="4" style="padding: 0rem 0.75rem 0rem 0.75rem;vertical-align: middle;"><input type="text" class="form-control" name="veritabani_secilen_yedekleme_oneki" id="veritabani_secilen_yedekleme_oneki" style="width:350px;" /></td>
        <td colspan="2">Yedeklenecek veri tabanına boşluksuz sadece Latin karakter ile bir isim vermelisiniz</td>
    </tr>
